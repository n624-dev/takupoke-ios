...
Web版「クラシック」を使用します。出典は `n624-dev/takupoke-astro` のコミット `e08b9d65c83613b12a90259d426bfcdd51e5c22f`、`public/takupoke-1024.png`（角丸マスクを付ける前の1024px素材）です。

- 「たくポケ」の文字、赤い線、鉛筆の配置と配色を基にしています。
- 白い背景・焼き込まれた影を分離し、文字・線・鉛筆の面を均一な色にしています。元画像の色領域を抽出したPNGを使用します。
- 前景は `Takupoke/AppIcon.icon/Assets/` 内の `ClassicArtwork.png`（赤い輪と鉛筆）、`ClassicTaku.png`、`ClassicPoke.png` の3レイヤーです。
- 文字を必ずアイコン中央に置きます。1024px素材上で「たく」「ポケ」の各行の可視範囲の中心をx=512にそろえ、2行全体の上端267px・下端757pxから縦の中心もy=512にしています。鉛筆を含む外形の中心に合わせて文字を移動しません。
- 文字の位置を基準に赤い輪を拡大・配置し、鉛筆は縦横比を保って8度起こし、先端と赤い線のつながりを維持します。元の輪の内側に残った抽出時の小さな点も除去しました。
- 円形と角丸のプレビュー、大きな表示と60px相当の表示で確認します。円形の外へ前景が欠けないこと、文字と輪・鉛筆が重ならないことを画素でも検査します。円形マスクは確認用で、アプリ素材自体には焼き込みません。
