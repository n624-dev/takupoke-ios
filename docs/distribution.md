...
この文書は実装した配布経路の手順です。CI でビルドと IPA・Source 生成は成功し、公開時の不具合を修正して再確認待ちです。初回導入・更新・データ保持の実機確認は未完了です。詳細は [検証記録](verification.md) を参照してください。
...
Source URL は初回の公開成功後から利用できます。GitHub Pages や独自サーバーの設定は不要です。
...
この経路で少なくとも一度の更新を確認するまで、ロードマップ①は完了扱いにしません。
