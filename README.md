...
**資料取得・XLSX/PDF解析に加え、保存済みの解析結果を見る SwiftUI の時間割タブを追加した開発版です。週グリッドでは通常時間割と日付・クラス・時限が一致する変更を切り替え、授業詳細に置き換え前の通常授業を表示します。学校行事を使った特別日判定、採用版を保存する統合処理、通知は未実装です。iOS ビルドと実機表示は確認待ちです。**
...
| SwiftUI 時間割タブ | 実装済み。iOS SDK ビルド・実機表示は未確認 |
