...
**資料取得・XLSX解析に加え、通常時間割と学校行事のPDF解析・結果確認画面を追加した開発版です。文字順の修正後、学校行事の解析完了が報告されました。通常時間割の行分けエラーに対して、読み順が一致する文字列の境界の重なりを扱う修正を追加しました。通常時間割の表示からPDF由来の改行を除きます。修正版の実機確認と結果の正確性の確認は継続中です。通常時間割・行事・変更の統合と通知は未実装です。**
...
| PDF解析・結果確認 | 学校行事は文字順修正後の解析完了報告あり。通常時間割のP17エラーに対し、文字列境界の重なり判定を修正。実機確認待ち。学校行事の追加変更は利用者の次の指定まで保留 |
