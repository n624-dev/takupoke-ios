...
**資料取得・XLSX解析に加え、通常時間割と学校行事のPDF解析・結果確認画面を追加した開発版です。実機でPDF解析は完了しましたが、文字順の誤りが報告されました。行・文字順の保持を修正し、行事タグを追加しています。修正版の実機での正確性は確認待ちです。通常時間割・行事・変更の統合と通知は未実装です。**
...
| PDF解析・結果確認 | 実機で解析完了後に文字順の誤りを報告。行・文字順の保持を修正し、タグを追加。修正版の実機での正確性は確認待ち |
