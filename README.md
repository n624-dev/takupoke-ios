...
**資料取得・XLSX解析に加え、通常時間割と学校行事のPDF解析・結果確認画面を追加した開発版です。PDF解析の共通処理はLinuxで検証済みですが、PDFKit経由のiOSビルド・実機確認はまだです。通常時間割・行事・変更の統合と通知は未実装です。**
...
| PDF解析・結果確認 | 実装追加。架空データと非公開資料の位置情報を使ってLinuxで検証。PDFKit経由のiOSビルド・実機確認待ち |
