...
[インストール手順](docs/altstore-pal-install.md) · [PC経由の導入・配布の仕組み](docs/distribution.md) · [開発環境](docs/development.md) · [ロードマップ](docs/roadmap.md) · [Releases](https://github.com/n624-dev/takupoke-ios/releases)
...
- 一覧タブで認証後に取得した `/links` のカテゴリ・検索・リンク起動を利用。設定でHTTPSリンクの外部／アプリ内表示を選べ、長押しからその1回だけ逆の方法で開けます。お気に入り・色変更・非表示は端末内に保存し、非表示のリンクは個別に復元できます。通信失敗時は保存期間内の一覧を表示します。取得・保存の条件は[一覧タブの仕様](docs/links-tab.md)を参照。
- 試験・返却の授業詳細は科目・教員・教室をラベル付きで表示。クラスや表示設定は更新後も保持し、資料に候補がない間も保存した選択を削除しない。
- PDFの記載名を保持し、元PDFもアプリ内で確認。正式名称の対応表と一覧は、一回の学校アカウント認証で取得・更新できます。
...
| 対応表の認証付き配信・表示への適用 | 実装済み。利用者から取得・表示の成功報告あり |
| 通知・バックグラウンド更新 | 未実装 |
