...
**資料取得・XLSX/PDF解析に加え、SwiftUI の時間割タブを実装した開発版です。保存済みの通常時間割・学校行事・時間割変更を週表示に反映し、試験・返却PDFは「ファイル」から選んで端末内で解析します。変更詳細には置き換え前の授業も残します。採用版の保存、通知、週画像の共有は未実装です。既存版の時間割タブは利用者から表示成功の報告を受けました。今回の追加部分は iOS SDK と実機での確認待ちです。**
...
- 時間割タブでクラス設定（1年生はホームルームと学科の組み合わせ）、前週／翌週、通常／変更込み、週の授業グリッド、授業詳細、変更一覧の「今日以降／この週／全件」を利用。試験・返却PDFの個別選択と解析にも対応。適用期間と未対応部分は[時間割タブの表示仕様](docs/timetable-tab.md)を参照。
...
| SwiftUI 時間割タブ | 既存版は iOS SDK ビルド・Release 公開と利用者の実機表示報告あり。学校行事・試験・返却など今回の追加部分は iOS SDK・実機確認待ち |
...
アプリの「学校資料を選ぶ」で通常時間割PDFと時間割変更XLSXを個別に選びます。試験・返却PDFは時間割タブの「試験・返却資料」から個別に選びます。学校行事は「学校サイトから取得」で保存し、以降は「更新を確認」を使います。起動時にPDFを毎回ダウンロードする処理はありません。再取得・アクセス失効時の対応・現在の制約は [学校資料の選択・取得](docs/materials.md) を参照してください。
