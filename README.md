...
- iOS「ファイル」からフォルダ、PDF、XLSXを選択し、資料の用途を指定して端末内に保存。
...
| File Provider からの資料取得 | CIビルド成功。公開処理の追加修正後の結果と、OneDrive の実機動作は確認待ち |
...
アプリの「学校資料を選ぶ」からフォルダを選び、通常時間割・学校行事・時間割変更に使うファイルを指定します。フォルダが選べない場合は個別のPDF・XLSXも選べます。再取得・アクセス失効時の対応・現在の制約は [学校資料の選択・取得](docs/materials.md) を参照してください。
...
確認メモは端末内の UserDefaults、資料とアクセス用ブックマークはアプリ専用の保存領域に置きます。資料の保存領域はバックアップから除外します。学校資料やアクセス情報を開発者のサーバーへ送信する処理はありません。OneDrive からの取得に必要な通信は File Provider が担当します。元資料は変更しません。
