...
**資料取得・XLSX解析に加え、通常時間割と学校行事のPDF解析・結果確認画面を追加した開発版です。文字順の修正後、学校行事の解析完了が報告されました。実機の診断情報で通常時間割のP20を再現し、過大な選択範囲を文字の位置として扱っていたことを確認しました。文字単位の矩形へ切り替えた後、実機でP01が報告されています。原因調査のため、通常時間割PDFの全文・文字位置・読み順・罫線・処理経過をまとめてコピーする機能を追加しました。P01の解消は未確認です。通常時間割の表示からPDF由来の改行を除きます。修正版の実機確認と結果の正確性の確認は継続中です。通常時間割・行事・変更の統合と通知は未実装です。**
...
| PDF解析・結果確認 | 学校行事は文字順修正後の解析完了報告あり。実機の診断で通常時間割のP20を再現。文字位置の修正後にP01の報告あり。全文診断のコピーを追加し、実機確認待ち。学校行事の追加変更は利用者の次の指定まで保留 |
