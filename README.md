...
**資料取得・XLSX解析に加え、通常時間割と学校行事のPDF解析・結果確認画面を追加した開発版です。実機で両PDFの書式判定エラーが報告され、文字位置の取得方法と失敗箇所の表示を修正しています。修正版の実機解析成功は未確認です。通常時間割・行事・変更の統合と通知は未実装です。**
...
| PDF解析・結果確認 | 実機で両PDFの書式判定エラー報告あり。文字位置の取得と失敗箇所の表示を修正。修正版の実機解析成功は確認待ち |
