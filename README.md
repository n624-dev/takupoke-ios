...
**資料取得・XLSX/PDF解析に加え、保存済みの解析結果を見る SwiftUI の時間割タブを追加した開発版です。週グリッドでは通常時間割と日付・クラス・時限が一致する変更を切り替え、授業詳細に置き換え前の通常授業を表示します。学校行事を使った特別日判定、採用版を保存する統合処理、通知は未実装です。iOS ビルドと配布は Actions で成功し、実機表示は確認待ちです。**
...
- 時間割タブで基本1クラス（1年生はホームルームと学科の組み合わせ）、前週／翌週、通常／変更込み、週の授業グリッド、授業詳細、変更一覧の「今日以降／この週／全件」を利用。適用期間と未対応部分は[時間割タブの表示仕様](docs/timetable-tab.md)を参照。
...
| SwiftUI 時間割タブ | 実装済み。iOS SDK ビルドと Release 公開が成功。実機表示は未確認 |
