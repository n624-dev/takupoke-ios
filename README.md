...
SQLite保存基盤を追加しました。開発版では旧JSONからのデータ移行を必須にせず、SQLite切替時に資料を再取得・再解析して保存し直す方針です。既存の移行候補作成処理はアプリから使いません。アプリの保存先切替・時間割統合はまだ行っていません。実装範囲と確認待ちの条件は[SQLite設計](docs/local-database-design.md)に記載しています。
...
| PDF解析・結果確認 | 通常時間割は描画命令から文字位置を計算するバージョン7へ変更。ローカル検証済み、実機確認待ち。学校行事は解析完了報告のある既存処理を維持し、追加変更は利用者の次の指定まで保留 |
