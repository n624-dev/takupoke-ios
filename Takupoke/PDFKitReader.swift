    ...
    private static func readPages(_ url: URL, kind: MaterialKind, diagnostics: PDFDiagnosticRecorder?,
        ...
        for index in 0..<document.pageCount {
            ...
            diagnostics?.record(.pageText, page: index + 1,
                values: [Double(ns.length), Double(page.numberOfCharacters), Double(rotation)], bounds: media)
                    ...
                    if kind == .timetable {
                        ...
                        do {
                            box = try PDFCharacterGeometry.bounds(for: range, count: ns.length) {
                                let rect = page.characterBounds(at: $0)
                                diagnostics?.record(.characterBounds, page: index + 1, index: $0, bounds: rect)
                                return rect
                            }
